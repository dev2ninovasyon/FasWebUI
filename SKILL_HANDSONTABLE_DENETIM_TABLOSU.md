---
name: handsontable-denetim-standartlari
...
Tablo satırlarının içeriğe göre kontrolsüz büyümesini engelleymek için şu CSS ve Props kuralları uygulanmalıdır:
...
Hücreler kompakt olduğu için, tıklanan hücrenin tüm içeriği tablonun altında ayrı bir panelde gösterilmelidir.
...
`@handsontable/react` bileşeni prop değişimlerine çok duyarlıdır. React infinite loop (Maximum update depth exceeded) hatasını önlemek için:

1.  **useCallback**: Tüm `afterChange`, `afterSelectionEnd` ve `beforeKeyDown` fonksiyonları `useCallback` ile sarılmalıdır.
