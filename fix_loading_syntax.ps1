...
foreach ($f in $files) {
    try {
        ...
        if ($content -match "<iframe") {
            ...
            # 1. Hatalı state yerleşimini düzelt (import ile component arasına girmiş olabilir)
            # Yanlış yeri bulup temizle
            $content = $content -replace "import \{ generateSignature \} from \"@/utils/crypto\";\s+const \[isLoading, setIsLoading\] = useState\(true\);", "import { generateSignature } from \"@/utils/crypto\";"
            
            # Doğru yeri bulup ekle (eğer yoksa)
            ...
            # 3. Yükleniyor Box'ı eğer eksikse iframe'in hemen üstüne ekle (relative olan grid'in içine)
            ...
            Write-Host "Fixed syntax in: $($f.Name)"
        }
    }
    catch {
