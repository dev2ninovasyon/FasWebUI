...
const Page = () => {
    ...
    const documentTitle = searchParams.get('title') || "Kredi Çalışması";
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            // User verileri henüz yüklenmemişse bekle
            ...
            try {
                ...
                const dipnotNo = await getDipnotNoByDipnotAdi(
                    ...
                    user.yil || 0,
                    "Kredi Hesaplama",
                    user.denetimTuru === "Tfrs" // MVC mantığına göre Bobi ise false gider
                );

                if (dipnotNo) {
                    // setDip(dipnotNo);
                } else {
                    // Eğer veritabanında "Kredi Hesaplama" adında dipnot yoksa fallback olarak 19 dene
                    console.error("Dipnot bulunamadı, manuel '19' denenebilir.");
                    // setDip("19");
                }
            } catch (error) {
    ...
    const BCrumbList = useMemo(() => {
        return [
            ...
            { to: currentPath, title: "Kredi Çalışması" },
        ];
    }, [basePath, currentPath, dip, parentName]);

    return (
        <PageContainer title="Kredi Çalışması" description="Kredi Çalışması">
            <Breadcrumb title="" subtitle="Kredi Çalışması" items={BCrumbList}>
