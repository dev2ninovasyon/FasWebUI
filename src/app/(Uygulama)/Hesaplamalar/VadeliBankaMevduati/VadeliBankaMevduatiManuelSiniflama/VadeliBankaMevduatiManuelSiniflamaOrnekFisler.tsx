...
const VadeliBankaMevduatiManuelSiniflamaOrnekFisler: React.FC<Props> = ({
  json,
  kaydetTiklandimi,
  setkaydetTiklandimi,
}) => {
  ...
  const fetchData = async () => {
    try {
      if (json) {
        const vadeliBankaMevduatiManuelSiniflamaOrnekFisVerileri =
          await getVadeliBankaMevduatiManuelSiniflamaOrnekFisler(user.denetciId || 0,
            user.yil || 0,
            user.denetlenenId || 0,
            json
          );
        ...
        const rowsAll: any = [];
        vadeliBankaMevduatiManuelSiniflamaOrnekFisVerileri.forEach(
          (veri: any) => {
            const newRow: any = [
              true,
              veri.yevmiyeNo,
  ...
  const handleDownload = () => {
    ...
    async function createExcelFile() {
      ...
      try {
        ...
        saveAs(blob, `VadeliBankaMevduatiManuelSiniflamaOrnekFisler.xlsx`);
...
export default VadeliBankaMevduatiManuelSiniflamaOrnekFisler;
