...
interface Veri {
  ...
  ay69: number;
  ay912: number;
  birYildanFazla: number;
  besYildanFazla: number;
  toplam: number;
}
...
const Yaslandirma: React.FC<Props> = ({ hesaplaTiklandimi }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const rowsAll: any = [];
      yaslandirmaVerileri.forEach((veri: any) => {
        const newRow: any = [
          veri.kebirKodu,
          veri.ay03,
          veri.ay36,
  ...
  return (
    <>
      <CustomHotTable theme={customizer.activeMode === "dark" ? "ht-theme-horizon-dark" : "ht-theme-horizon"}
        ...
        columns={columns}
        colWidths={[40, 50, 50, 50, 50, 50, 50, 50]}
        stretchH="all"
