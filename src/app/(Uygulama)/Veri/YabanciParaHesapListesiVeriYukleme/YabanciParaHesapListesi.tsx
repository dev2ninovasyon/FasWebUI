...
import {
  createMultipleYabanciParaHesapListesiVerisi,
  createNullYabanciParaHesapListesiVerisi,
  createYabanciParaHesapListesiVerisi,
  deleteYabanciParaHesapListesiVerisi,
  getYabanciParaHesapListesiVerileriByDenetciDenetlenenYil,
...
const YabanciParaHesapListesi = () => {
  ...
  const handleCreateRow = async (index: number, amount: number) => {
    if (amount == 1 && index != rowCount - 1) {
      ...
      setControl(true);
      await handleUpdateMultipleYabanciParaHesapListesiVerisi(index);
      await handleCreateNullYabanciParaHesapListesiVerisi(index);
    }
  };
  ...
  const handleAfterRemoveRow = async (
    ...
    handleDeleteYabanciParaHesapListesiVerisi(siraNos);
  };
  ...
  const handleAfterChange = async (changes: any, source: any) => {
    ...
    if (changes) {
      for (const [row, prop, oldValue, newValue] of changes) {
        ...
        if (
          ...
          await handleUpdateYabanciParaHesapListesiVerisi(changedRow);
    ...
    if (
      ...
      await handleCreateYabanciParaHesapListesiVerisi(changedRow);
      changedRow = -1;
      changedRows = [];
    }
    //Çok Satır Yaratma
      ...
      //Yarım Yarım Çok satır Yaratma
        ...
        for (const x of changedRows) {
          if (
            ...
            await handleCreateYabanciParaHesapListesiVerisi(x);
          }
        }
  ...
  const handleCreateYabanciParaHesapListesiVerisi = async (row: number) => {
    const rowData = await handleGetRowData(row);
    if (rowData[4] == null || rowData[4] == "") {
      rowData[4] = "TL";
    }

    const createdYabanciParaHesapListesiVerisi = {
      ...
      hesapAdi: rowData[2],
      detayHesapAdi: rowData[3],
      paraBirimi: rowData[4],
      mutlakBakiye: rowData[5],
      dovizBakiye: rowData[6],
    };

    try {
      const result = await createYabanciParaHesapListesiVerisi(
        user.token || "",
        createdYabanciParaHesapListesiVerisi
      );
      if (result) {
        if (halfPasteControl == false) {
          await fetchData();
        }
        console.log("Yabancı Para Hesap Listesi Verisi ekleme başarılı");
      } else {
        console.error("Yabancı Para Hesap Listesi Verisi ekleme başarısız");
      }
    } catch (error) {
  ...
  const handleCreateMultipleYabanciParaHesapListesiVerisi = async () => {
    const keys = [
      "denetciId",
      "denetlenenId",
      "yil",
        ...
        ? keys.reduce(
            (acc: { [key: string]: any }, key: string, index: number) => {
              if (key === "denetciId") {
                ...
                acc[key] = user.yil;
              } else if (key === "mutlakBakiye" || key === "dovizBakiye") {
                acc[key] = parseFloat(
    ...
    try {
      const result = await createMultipleYabanciParaHesapListesiVerisi(
        user.token || "",
        jsonData
      );
      if (result) {
  ...
  const handleCreateNullYabanciParaHesapListesiVerisi = async (
    siraNo: number
  ) => {
    try {
      const result = await createNullYabanciParaHesapListesiVerisi(
        user.token || "",
        user.denetciId || 0,
        user.denetlenenId || 0,
        user.yil || 0,
  ...
  const handleUpdateYabanciParaHesapListesiVerisi = async (row: number) => {
    const rowData = await handleGetRowData(row);
    if (rowData[4] == null || rowData[4] == "") {
      rowData[4] = "TL";
    }
    const updatedYabanciParaHesapListesiVerisi = {
      detayHesapKodu: rowData[1],
      hesapAdi: rowData[2],
      detayHesapAdi: rowData[3],
      paraBirimi: rowData[4],
    ...
    try {
      const result = await updateYabanciParaHesapListesiVerisi(
        user.token || "",
        user.denetciId || 0,
        user.denetlenenId || 0,
        user.yil || 0,
        row + 1,
        updatedYabanciParaHesapListesiVerisi
      );
      if (result) {
        await fetchData();
        console.log("Yabancı Para Hesap Listesi Verisi güncelleme başarılı");
      } else {
        console.error("Yabancı Para Hesap Listesi Verisi güncelleme başarısız");
      }
    } catch (error) {
      console.error("Bir hata oluştu:", error);
    }
  ...
  const handleUpdateMultipleYabanciParaHesapListesiVerisi = async (
    siraNo: number
  ) => {
    try {
      const result = await updateMultipleYabanciParaHesapListesiVerisi(
        user.token || "",
        user.denetciId || 0,
        user.denetlenenId || 0,
        user.yil || 0,
  ...
  const handleDeleteYabanciParaHesapListesiVerisi = async (
    siraNo: number[]
  ) => {
    try {
      const result = await deleteYabanciParaHesapListesiVerisi(
        user.token || "",
        user.denetciId || 0,
        user.denetlenenId || 0,
        user.yil || 0,
      ...
      if (result) {
        ...
        console.log("Yabancı Para Hesap Listesi Verisi silme başarılı");
      } else {
        console.error("Yabancı Para Hesap Listesi Verisi silme başarısız");
      }
    } catch (error) {
      console.error("Bir hata oluştu:", error);
    }
  ...
  const fetchData = async () => {
    try {
      const yabanciParaHesapListesiVerileri =
        await getYabanciParaHesapListesiVerileriByDenetciDenetlenenYil(
          user.token || "",
          user.denetciId || 0,
          user.denetlenenId || 0,
          user.yil || 0
      ...
      const rowsAll: any = [];
      yabanciParaHesapListesiVerileri.forEach((veri: any) => {
        const newRow: any = [
          veri.siraNo,
          veri.detayHesapKodu,
          veri.hesapAdi,
  ...
  useEffect(() => {
    if (afterPasteCompleted) {
      handleCreateMultipleYabanciParaHesapListesiVerisi();
  ...
  const handleDownload = () => {
    ...
    async function createExcelFile() {
      ...
      try {
        ...
        saveAs(blob, "YabanciParaHesapListesiFormati.xlsx");
  ...
  return (
    <>
      <HotTable
        ...
        columns={columns}
        colWidths={[0, 80, 120, 120, 80, 100, 100]}
        stretchH="all"
...
export default YabanciParaHesapListesi;
