...
const SatinAlmaOdemeKontrolCalismaKagidi: React.FC<CalismaKagidiProps> = ({
  controller,
  grupluMu,
...
const [receivedRows, setReceivedRows] = useState<ReceivedInvoiceMatchRow[]>([]);
const rcvHotRef = useRef<any>(null);

const loadReceivedMatches = async () => {
  try {
    const data = await getReceivedInvoiceMatches(user);
    setReceivedRows(data);
  } catch (e: any) {
    enqueueSnackbar(e?.message || "Alınan eşleşmeleri alınamadı", { variant: "error" });
  }
};
  ...
  () =>
    receivedRows.map((r) => [
      ...
      r.invoiceDate
        ...
        : "",
      r.counterpartyName ?? "",      // Tedarikçi
      r.counterpartyVkn ?? "",       // Tedarikçi VKN
      r.amount ?? 0,
      r.kdv ?? 0,
      r.yevmiyeNo ?? "",
      r.yevmiyeDate
...
// NOT: type:"numeric" hatası yaşamamak için kolon tiplerini string/numeric objeleriyle veriyoruz:
const receivedColumns: any[] = [
  { readOnly: true },                // _MatchId_
  { type: "text", readOnly: true },  // Fatura No
  { type: "text", readOnly: true },  // Fatura Tarihi
  { type: "text", readOnly: true },  // Tedarikçi
  { type: "text", readOnly: true },  // Tedarikçi VKN
  { type: "numeric", readOnly: true, numericFormat: { pattern: "0,0.00", culture: "tr-TR" } }, // Tutar
  { type: "numeric", readOnly: true, numericFormat: { pattern: "0,0.00", culture: "tr-TR" } }, // KDV
  { type: "text", readOnly: true },  // Yevmiye No
  { type: "text", readOnly: true },  // Yevmiye Tarihi
];
    ...
    <Card>
      ...
      <CardContent>
        <HotTable
          ...
          hiddenColumns={{ columns: [0], indicators: false }} // matchId gizli
          stretchH="all"
          rowHeaders
          height={360}
...
export default SatinAlmaOdemeKontrolCalismaKagidi;
