...
const AmortismanCari: React.FC<Props> = ({
  dipnotKodu,
  kaydetTiklandimi,
  setKaydetTiklandimi,
  konsolide,
  ...
  const columns = [
    ...
    { type: "text", columnSorting: true, className: "htLeft" }, // Başlık
    {
      type: "numeric",
      numericFormat: {
        pattern: "0,0.00",
  ...
  const handleSaveAmortismanVerisi = async () => {
    const keys = [
      "denetciId",
      "denetlenenId",
      "yil",
    ...
    if (result) {
      ...
      console.log("Dipnot Amortisman Cari güncellenemedi.");
    }
  };
  ...
  const fetchData = async () => {
    try {
      if (dipnotKodu == 14 || dipnotKodu == 15) {
        ...
        const rowsAll: any = [];
        if (amortismanVerileri && Array.isArray(amortismanVerileri)) {
          amortismanVerileri
  ...
  return (
    <>
      <HotTable
        ...
        columns={columns}
        colWidths={[120, 100, 100, 100, 100, 100, 100, 100]}
        stretchH="all"
